...
// GET - Obtener un pedido por ID junto con los productos
...
// GET - Obtener pedidos por ID de usuario
    ...
    connection.query(query, [userId], (err, results) => {
        ...
        results.forEach(row => {
            if (!pedidosMap.has(row.ID_Pedido)) {
                ...
                pedidosMap.set(row.ID_Pedido, pedido);
                pedidos.push(pedido);
            }
...
const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (response.status !== 201) {
            throw new Error(response.data.error || 'Error al crear la tienda');
        }
        ...
        message.error(error.message || "Error al crear la tienda");
    }
};
